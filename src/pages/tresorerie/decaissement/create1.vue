                ...
                <div class="control-group mt-2 p-2" id="app">
                  <div class="row">
                    <div class="form-group col-md-9">
                      ...
                      <input :class="{ 'is-invalid': formErrors.errorMotif }" type="text" class="form-control"
                        id="exampleInputEmail1" placeholder="Achat de matériels" v-model="decaisse.motif" />
                      <div class="invalid-feedback" v-if="formErrors.errorMotif">
                        {{ formErrors.errorMotif }}
                      </div>
                    </div>

                    <div class="form-group col-md-3">
                      <label for="montant">Montant</label>
                      <input :class="{ 'is-invalid': formErrors.errorMontant }" type="text" class="form-control"
                        id="montant" placeholder="100 000" v-model="decaisse.montant" />
                      <div class="invalid-feedback" v-if="formErrors.errorMontant">
                        {{ formErrors.errorMontant }}
                      </div>
                    </div>
                  </div>
                </div>
                ...
                <div class="form-group col-xs-12">
                  ...
                  <div class="input-group">
                    <div class="custom-file">
                      <input :class="{ 'is-invalid': formErrors.errorFacture }" accept="*"
                        v-on:change="handleFileChange" type="file" class="file-input " id="fileInput">
                      <label class="custom-file-label" for="fileInput">{{ fileName }}</label>
                    </div>
                    <!-- <div class="custom-file-container custom-file">
                      <input :class="{ 'is-invalid': formErrors.errorFacture }" accept=".pdf,.doc,.docx"
                        v-on:change="handleFileChange" type="file" class="file-input" id="fileInput">
                      <label class="custom-file-label" for="fileInput">{{ fileName }}</label>
                    </div> -->
      ...
      decaissementService
        ...
        .catch((error) => {
          ...
          if (!this.decaisse.motif) {
            this.formErrors.errorMotif = "Le motif du décaissmente est requis";
            isValid = false;
          }

          if (!this.decaisse.montant) {
            this.formErrors.errorMontant =
              "Le montant du décaissement est requis.";
            isValid = false;
          }

          if (!this.decaisse.facture) {
            this.formErrors.errorFacture =
              "La preuve du décaissement est requis.";
            isValid = false;
          }

          if (!this.decaisse.description) {
            this.formErrors.errorDescription =
              "La description du décaissement est requise.";
            isValid = false;
          }
          if (!isValid) {
            return;
...
.custom-file-label::after {
  content: 'Télécharger';
  display: block;
  background: linear-gradient(to bottom, #007bff, #0056b3); /* Dégradé de bleu */

  color: #f7f1f1; /* Couleur du texte du libellé */
